...
const GrupoProduto = () => {
    ...
    const getGrupoProduto = () => {
        fetch("http://localhost:8080/api/grupoProduto")
    ...
    const handleChange = (e) => {
        ...
        setGrupoProduto({ ...grupoProduto, [name]: value });
    }
    const handleRadio = (e) => {
        const isSelected = e.target.value === "true" ? true : false;
        setGrupoProduto({ ...grupoProduto, ativo: isSelected });

    }
    const handleSubmit = (e) => {
        e.preventDefault();
        if (grupoProduto.id === 0) {
            fetch("http://localhost:8080/api/grupoproduto", {
                method: "POST",
                mode: "cors",
            ...
            const id = grupoProduto.id;
            fetch(`http://localhost:8080/api/grupoproduto/${id}`, {
                method: "PUT",
                mode: "cors",
                body: JSON.stringify(grupoProduto),
                headers: {
        ...
        setGrupoProduto(initialForms);

    }
    const handleUpdate = (id) => {
        fetch(`http://localhost:8080/api/grupoproduto/${id}`)
            .then(response => response.json())
            .then(data => setGrupoProduto(data))
            ...
            <Container>
                <Main>
                    ...
                    <Form>
                        <FormGroup>
                            <Label name="Nome">
                                <Input type="text" name="nome" value={grupoProduto.nome} placeholder="Informe o nome" onchange={handleChange} />
                            </Label>
                        </FormGroup>
                        <FormRadio>
                            <Label name="Ativo" />
                            <Input type="radio" name="ativo" value="true" valorSelecionado={grupoProduto.ativo === true} onchange={handleRadio} /><Span name="Sim" />
                            <Input type="radio" name="ativo" value="false" valorSelecionado={grupoProduto.ativo === false} onchange={handleRadio} /><Span name="Não" />
                        </FormRadio>
